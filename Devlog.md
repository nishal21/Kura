...
- Fixed release asset upload collisions (`kura.png already exists` / intermittent upload 404s) by downloading artifacts into separate directories (`merge-multiple: false`) and packaging each OS artifact set into a unique archive (`tauri-release-<os>-<tag>.tar.gz`) before `gh release upload --clobber`.
- Refined production release assets to include installer outputs only: build jobs now collect and upload only distributable files (`.AppImage`, `.deb`, `.rpm`, `.dmg`, `.msi`, `.exe`) instead of full bundle internals.
- Updated publish behavior to auto-recreate existing draft releases before upload, which clears previously cluttered assets in one API operation while preserving safe in-place updates for already published releases.
- Reworked `README.md` into a release-first product guide with clear installer selection by OS, prominent official `v1.0.0` release links, improved onboarding flow, stronger feature narrative, updated troubleshooting, and cleaner developer documentation structure.
